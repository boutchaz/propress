...
const itemSchema = z.object({
  ojdTag: z.string().nullable().optional(),
...
export const ItemEdit = ({
  ...
  const methods = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      releaseOffset: item.releaseOffset,
      goalFixedUpon: item.goalFixedUpon,
      publication: {
        value: item.publication["@id"],
      },
    },
  });
  useEffect(() => {
  ...
  const mutation = useMutation({
    mutationFn: async (data: FormValues & { section: string }) => {
      await kioskApi.updateSectionItem(item.id.toString(), data);
    },
    onSuccess: async () => {
      close();
